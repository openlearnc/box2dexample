...
int main(){
    ...
    if(!(window = SDL_CreateWindow("win", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1000, 800, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE))){
    ...
    printf("%d %d\n",screenw,screenh);
   SDL_Surface *surface = SDL_CreateRGBSurfaceWithFormat(0,800,1000,32,SDL_PIXELFORMAT_ARGB8888);
...
//ctx.setFillStyle(BLRgba32(0,255,255,125));
 ...
 ctx.setCompOp(BL_COMP_OP_SRC_OVER);
    ...
    ctx.fillRect((pos.x-boxw/2)*meter,(pos.y-boxh/2)*meter,boxw*meter,boxh*meter);
